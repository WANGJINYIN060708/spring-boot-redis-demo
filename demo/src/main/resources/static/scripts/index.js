(function($) {
	 ...
	 * 依据子系统ID加载子系统功能菜单
	 */
	window.mainpage.loadSystemNavigation = function(systemId) {
		...
		var url = 'sys/navigation/listModules.htm';
		$.get(url, {systemId: systemId}, function (menus) {
	...
	window.mainpage.loadSubSystems = function(callback) {
		...
		var url = 'sys/navigation/listSystem.htm';
					...
					var a = $("<a></a>").attr({
						...
						function() {
							...
							function() {
								...
								window.mainpage.loadSystemNavigation(item.id);
							}
						).appendTo(pp);
